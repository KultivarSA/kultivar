...
// Task #179 — Android status-bar icons must be monochrome alpha masks;
// the full-colour launcher icon renders as a flat white square.  This
// drawable set is generated by tools/make-notification-icon.py from the
// brand K foreground.
const String _kNotificationIcon = '@drawable/ic_stat_kultivar';

/// Task #179 — initialise the tz database AND pin `tz.local` to the
/// device's real timezone.
///
/// `tz.initializeTimeZones()` alone leaves `tz.local` at UTC, which
/// silently shifted every zonedSchedule (burping, watering, feeding,
/// IPM, drying, curing, target harvest) to UTC wall-clock — two hours
/// late for South African users, and the reason Marco's burping
/// reminder "never fired" (it was queued for 11:00 SAST instead of
/// 9:00).  Top-level so both the foreground service and the background
/// snooze isolate share it.
///
/// Falls back to UTC on lookup failure (e.g. exotic OEM timezone IDs
/// missing from the tz database) rather than crashing scheduling.
Future<void> configureLocalTimezone() async {
  ...
  try {
    ...
    // tz.local stays UTC -- reminders still fire, just offset.  Not
    // routed through ErrorReporter because this also runs in the
    // background isolate where no sink is installed.
  }
}
